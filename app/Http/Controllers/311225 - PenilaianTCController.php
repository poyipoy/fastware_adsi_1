...
class PenilaianTCController extends Controller
{
    ...
    public function dsCompetency()
    {
        ...
        if (in_array($currentUserRoleId, [1, 15])) {
            // Jika role_id adalah salah satu dari yang dikecualikan, ambil semua id_job_position dengan status 3 (approved)
            ...
            $jobPositions = TrsPenilaianTc::where('status', 3)
                ->where(function ($query) use ($currentUserRoleId) {
                    ...
                    } else if (in_array($currentUserRoleId, [14])) {
                        $query->whereIn('modified_at', [14,40, 41]);
    ...
    public function editTrs($id_job_position)
    {
        ...
        Log::info('Detail Penilaian Count:', [
            'id_job_position' => $id_job_position,
            'count' => $detailPenilaian->count(),
            'data' => $detailPenilaian->toArray()
        ]);
    ...
    public function updateTrs(Request $request, $id_job_position)
    {
        ...
        // Tidak mengubah status saat update nilai
        ...
        // Index terpisah untuk setiap kategori
        $tcIndex = 0;
        $skIndex = 0;
        $adIndex = 0;
        ...
        foreach ($penilaians as $penilaian) {
            ...
            $userName = $penilaian->user ? $penilaian->user->name : 'Unknown';

            // Proses nilai_tc
            if (isset($data['nilai_tc'][$tcIndex])) {
                $oldValue = $penilaian->nilai_tc;
                $newValue = $data['nilai_tc'][$tcIndex];
                
                if ($oldValue != $newValue) {
                    $penilaian->nilai_tc = $newValue;
                    $hasChanged = true;
                    $hasAnyChange = true;
                    $keterangan = $data['keterangan_tc'][$tcIndex] ?? 'N/A';
                    $currentKeteranganDetail[] = "TC: {$keterangan} ({$oldValue} → {$newValue})";
                }
                $tcIndex++;
            }

            // Proses nilai_sk
            if (isset($data['nilai_sk'][$skIndex])) {
                $oldValue = $penilaian->nilai_sk;
                $newValue = $data['nilai_sk'][$skIndex];
                
                if ($oldValue != $newValue) {
                    $penilaian->nilai_sk = $newValue;
                    $hasChanged = true;
                    $hasAnyChange = true;
                    $keterangan = $data['keterangan_sk'][$skIndex] ?? 'N/A';
                    $currentKeteranganDetail[] = "SK: {$keterangan} ({$oldValue} → {$newValue})";
                }
                $skIndex++;
            }

            // Proses nilai_ad
            if (isset($data['nilai_ad'][$adIndex])) {
                $oldValue = $penilaian->nilai_ad;
                $newValue = $data['nilai_ad'][$adIndex];
                
                if ($oldValue != $newValue) {
                    $penilaian->nilai_ad = $newValue;
                    $hasChanged = true;
                    $hasAnyChange = true;
                    $keterangan = $data['keterangan_ad'][$adIndex] ?? 'N/A';
                    $currentKeteranganDetail[] = "AD: {$keterangan} ({$oldValue} → {$newValue})";
                }
                $adIndex++;
            }
            ...
            if ($hasChanged) {
                $penilaian->modified_updated = auth()->user()->name;
                $penilaian->save();
        ...
        // Simpan ke DetailTcPenilaian jika ada perubahan
        if ($hasAnyChange && !empty($changesByName)) {
            foreach ($changesByName as $userName => $keteranganDetails) {
                DetailTcPenilaian::create([
                    ...
                    'catatan' => $data['alasan_perubahan'] ?? 'Tidak ada catatan',
            ...
            return response()->json([
                'success' => true, 
                'message' => 'Nilai berhasil diupdate dan history tersimpan'
            ]);
        } else {
    ...
    // Submit dari Draft ke Dept Head (Status 1 -> 2)
    public function kirimToDeptHead(Request $request, $id_job_position)
    {
        ...
        foreach ($penilaians as $penilaian) {
            $penilaian->status = 2; // Status: Menunggu Konfirmasi Dept Head
            $penilaian->modified_at = auth()->user()->id;
            $penilaian->save();
        }

        return response()->json(['success' => true, 'message' => 'Data Competency telah dikirim ke Dept Head.']);
    }

    // Approve dari Dept Head (Status 2 -> 3)
    public function approveData(Request $request, $id_job_position)
    {
        $penilaians = TrsPenilaianTc::where('id_job_position', $id_job_position)->get();
        ...
        foreach ($penilaians as $penilaian) {
            $penilaian->status = 3; // Status: Telah Disetujui
            $penilaian->modified_at = auth()->user()->id;
            $penilaian->save();
        }

        return response()->json(['success' => true, 'message' => 'Data Competency telah disetujui.']);
    }
    //chartRadar
