...
@section('content')
    <main id="main" class="main">
        ...
        <style>
            ...
            .progress-summary-track:focus-visible {
                border-radius: 8px;
            ...
            .progress-summary-pill strong::before {
            ...
            .progress-summary-pill.status-vendor { background: #eef6ff; border-color: #b9d7ff; color: #075dcc; }
            .progress-summary-pill.status-registration { background: #fff4e9; border-color: #ffd0a4; color: #b84d00; }
            .progress-summary-pill.status-progress { background: #fff8df; border-color: #f5d378; color: #805b00; }
            .progress-summary-pill.status-done { background: #eaf7f1; border-color: #acdcca; color: #08754c; }
            .progress-summary-pill.status-pending { background: #f1f3f5; border-color: #d5d9dd; color: #4f5963; }
            .progress-summary-pill.status-rejected { background: #fff0f1; border-color: #ffc2c7; color: #c51d2b; }
        ...
        <div class="blue-theme-header d-flex justify-content-between align-items-center flex-wrap gap-3">
            ...
            <div class="d-flex align-items-center gap-2">
                <a href="{{ route('people_development.export.approvals', request()->query()) }}"
                    class="btn btn-outline-light rounded-pill px-4 py-2 fw-bold {{ $data->isEmpty() ? 'disabled' : '' }}"
        ...
        <div class="container-fluid px-0">
            @php
                $progressTotal = max((int) ($kpiTotalTraining ?? 0), 0);
                $progressItems = [
                    ['label' => 'Mencari Vendor', 'count' => $kpiMencariVendor ?? 0, 'class' => 'status-vendor'],
                    ['label' => 'Proses Pendaftaran', 'count' => $kpiProsesPendaftaran ?? 0, 'class' => 'status-registration'],
                    ['label' => 'On Progress', 'count' => $kpiOnProgress ?? 0, 'class' => 'status-progress'],
                    ['label' => 'Done', 'count' => $kpiDone ?? 0, 'class' => 'status-done'],
                    ['label' => 'Pending', 'count' => $kpiPending ?? 0, 'class' => 'status-pending'],
                    ['label' => 'Ditolak', 'count' => $kpiDitolak ?? 0, 'class' => 'status-rejected'],
                ];
            @endphp

            <section class="progress-summary" aria-labelledby="progress-summary-title">
                <h2 class="progress-summary-title" id="progress-summary-title">
                    <i class="bi bi-pie-chart-fill text-secondary me-2" aria-hidden="true"></i>Ringkasan Status
                </h2>
                <div class="progress-summary-track" role="list" tabindex="0"
                    aria-label="Status progres training. Geser ke samping untuk melihat seluruh status.">
                    @foreach ($progressItems as $progressItem)
                        @php
                            $progressCount = (int) $progressItem['count'];
                            $progressPercentage = $progressTotal > 0
                                ? (int) round(($progressCount / $progressTotal) * 100)
                                : 0;
                        @endphp
                        <div class="progress-summary-pill {{ $progressItem['class'] }}" role="listitem">
                            <span>{{ $progressItem['label'] }}</span>
                            <strong>{{ $progressCount }} ({{ $progressPercentage }}%)</strong>
                        </div>
                    @endforeach
                </div>
            </section>
                ...
                <div class="col-6 col-md-3">
                    <div class="card kpi-card h-100">
                        <div class="card-body p-3 d-flex align-items-center gap-3">
                            ...
                            <div>
                                <div class="text-muted small fw-bold text-uppercase" style="font-size:0.72rem;">Total Training</div>
                                <div class="fw-bold fs-4">{{ $kpiTotalTraining ?? 0 }}</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-6 col-md-3">
                    <div class="card kpi-card h-100">
                        <div class="card-body p-3 d-flex align-items-center gap-3">
                            ...
                            <div>
                                <div class="text-muted small fw-bold text-uppercase" style="font-size:0.72rem;">Budget Aktual</div>
