...
class BOPMController extends Controller
{
    ...
    public function exportTemplate(Request $request)
    {
        ...
        $materials = MstMaterial::query()
            ...
            ->select('mst_material.id', 'mst_material.grade', 'mst_shape.name as shape_name')
    ...
    public function importData(Request $request): JsonResponse
    {
        ...
        foreach ($rows as $idx => $row) {
            $rowNumber = $idx + 2; // heading row = 1
            $grade = trim((string)($row['nama_material'] ?? ''));
            $shapeName = trim((string)($row['shape'] ?? ''));
            ...
            $record = TrxQuartal::where('id_material', $material->id)
                ->where('thn', $year)
                ->first();

            if ($record && (
                !is_null($record->{$fieldBase}) ||
                !is_null($record->{$fieldAlloy}) ||
                !is_null($record->{$fieldFob}) ||
                !is_null($record->{$fieldCnf}) ||
                !is_null($record->{$fieldFreight})
            )) {
                $errors[] = "Baris {$rowNumber}: data {$quarter} tahun {$year} untuk material '{$grade}' sudah ada";
                continue;
            }

            $parsedFields = [
                $fieldBase => $this->parseNullableNumber($row['base'] ?? null),
                $fieldAlloy => $this->parseNullableNumber($row['alloy'] ?? null),
                $fieldFob => $this->parseNullableNumber($row['fob'] ?? null),
                $fieldCnf => $this->parseNullableNumber($row['cnf'] ?? null),
                $fieldFreight => $this->parseNullableNumber($row['freight'] ?? null),
            ];

            // Jika semua nilai kosong, lewati baris ini tanpa error
    ...
    private function buildChartData(?string $startDate, ?string $endDate, ?int $materialId, ?int $currencyId, float $multiplier = 1, ?int $startQuarter = null, ?int $startYear = null, ?int $endQuarter = null, ?int $endYear = null): array
    {
        ...
        $finalMultiplier = $kurs * $multiplier;
        ...
        return Cache::remember($cacheKey, now()->addMinutes(5), function () use ($yearStart, $yearEnd, $materialId, $finalMultiplier, $startQuarter, $startYear, $endQuarter, $endYear) {
            ...
            for ($year = $startYear; $year <= $endYear; $year++) {
                ...
                for ($q = $qStart; $q <= $qEnd; $q++) {
                    ...
                    if ($materialId === null) {
                        ...
                        $series['base'][$categoryIndex] = $yearData->sum($qBase) * $finalMultiplier ?: null;
                        $series['alloy'][$categoryIndex] = $yearData->sum($qAlloy) * $finalMultiplier ?: null;
                        $series['fob'][$categoryIndex] = $yearData->sum($qFob) * $finalMultiplier ?: null;
                        $series['cnf'][$categoryIndex] = $yearData->sum($qCnf) * $finalMultiplier ?: null;
                        $series['freight'][$categoryIndex] = $yearData->sum($qFreight) * $finalMultiplier ?: null;
                    } else {
                        // Single material
                        $record = $yearData->firstWhere('id_material', $materialId);
                        if ($record) {
                            $series['base'][$categoryIndex] = $record->$qBase * $finalMultiplier ?: null;
                            $series['alloy'][$categoryIndex] = $record->$qAlloy * $finalMultiplier ?: null;
                            $series['fob'][$categoryIndex] = $record->$qFob * $finalMultiplier ?: null;
                            $series['cnf'][$categoryIndex] = $record->$qCnf * $finalMultiplier ?: null;
                            $series['freight'][$categoryIndex] = $record->$qFreight * $finalMultiplier ?: null;
                        }
                    }
    ...
    private function buildTableData(?string $startDate, ?string $endDate, ?int $materialId, ?int $currencyId, float $multiplier = 1, ?int $startQuarter = null, ?int $startYear = null, ?int $endQuarter = null, ?int $endYear = null): Collection
    {
        ...
        return Cache::remember($cacheKey, now()->addMinutes(5), function () use ($yearStart, $yearEnd, $materialId, $finalMultiplier, $startQuarter, $startYear, $endQuarter, $endYear) {
            ...
            return $groupedByMaterial->map(function ($materialRows) use ($yearStart, $yearEnd, $finalMultiplier, $startQuarter, $startYear, $endQuarter, $endYear) {
                ...
                for ($year = $startYear; $year <= $endYear; $year++) {
                    ...
                    for ($q = $qStart; $q <= $qEnd; $q++) {
                        if ($yearData) {
                            $quarters[] = [
                                'period' => "Q{$q} {$year}",
                                'base' => (float) ($yearData->{"q{$q}_base"} ?: 0) * $finalMultiplier,
                                'alloy' => (float) ($yearData->{"q{$q}_alloy"} ?: 0) * $finalMultiplier,
                                'fob' => (float) ($yearData->{"q{$q}_fob"} ?: 0) * $finalMultiplier,
                                'cnf' => (float) ($yearData->{"q{$q}_cnf"} ?: 0) * $finalMultiplier,
                                'freight' => (float) ($yearData->{"q{$q}_freight"} ?: 0) * $finalMultiplier,
                            ];
                        } else {
