...
@section('content')
    <main id="main" class="main">
        ...
        <div class="pagetitle mb-4" data-training-status-anchor>
        ...
        <div class="container-fluid px-0">
            ...
            <div class="dashboard-header">
                ...
                <div class="d-flex align-items-center gap-3">
                    <a href="{{ route('people_development.export.submissions', request()->query()) }}"
                        class="btn btn-outline-success rounded-pill px-3 {{ $data->isEmpty() ? 'disabled' : '' }}"
            ...
            <div class="row mb-4 g-3">
                ...
                <div class="training-status-summary" data-training-status-summary>
                    @php
                        $trainingProgressTotal = max((int) ($kpiTotalTraining ?? 0), 0);
                        $trainingProgress = [
                            ['label' => 'Mencari Vendor', 'count' => $kpiMencariVendor ?? 0, 'style' => 'background-color: rgba(13, 110, 253, 0.1); color: #075dcc; border-color: rgba(13, 110, 253, 0.25);'],
                            ['label' => 'Proses Pendaftaran', 'count' => $kpiProsesPendaftaran ?? 0, 'style' => 'background-color: rgba(253, 126, 20, 0.1); color: #b84d00; border-color: rgba(253, 126, 20, 0.25);'],
                            ['label' => 'On Progress', 'count' => $kpiOnProgress ?? 0, 'style' => 'background-color: rgba(255, 193, 7, 0.15); color: #805b00; border-color: rgba(255, 193, 7, 0.35);'],
                            ['label' => 'Done', 'count' => $kpiDone ?? 0, 'style' => 'background-color: rgba(25, 135, 84, 0.1); color: #08754c; border-color: rgba(25, 135, 84, 0.25);'],
                            ['label' => 'Pending', 'count' => $kpiPending ?? 0, 'style' => 'background-color: rgba(108, 117, 125, 0.1); color: #4f5963; border-color: rgba(108, 117, 125, 0.25);'],
                            ['label' => 'Ditolak', 'count' => $kpiDitolak ?? 0, 'style' => 'background-color: rgba(220, 53, 69, 0.1); color: #c51d2b; border-color: rgba(220, 53, 69, 0.25);'],
                        ];
                    @endphp
                    <h6 class="fw-bold">
                        <i class="bi bi-pie-chart-fill me-2 text-secondary" aria-hidden="true"></i>Ringkasan Status
                    </h6>
                    <div class="d-flex" style="gap: 12px;">
                        @foreach ($trainingProgress as $progress)
                            @php
                                $statusCount = (int) $progress['count'];
                                $statusPercentage = $trainingProgressTotal > 0
                                    ? (int) round(($statusCount / $trainingProgressTotal) * 100)
                                    : 0;
                            @endphp
                            <div class="d-inline-flex align-items-center px-3 py-1"
                                style="border: 1px solid; border-radius: 20px; font-size: 13px; font-weight: 600; {{ $progress['style'] }}">
                                {{ $progress['label'] }}
                                <span class="mx-2" style="opacity: 0.3;">|</span>
                                <span>{{ $statusCount }} <span style="font-weight: 400; opacity: 0.8;">({{ $statusPercentage }}%)</span></span>
                            </div>
                        @endforeach
                    </div>
                </div>
