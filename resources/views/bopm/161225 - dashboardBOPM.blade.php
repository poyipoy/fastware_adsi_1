...
<main id="main" class="container-fluid py-4">
    <div class="pagetitle mb-4">
            <h1 class="mb-1">Dashboard BOPM</h1>
            <nav>
        ...
        <section class="card shadow-sm mb-4">
            <div class="card-body p-4" style="background: #6c757d">
                <form id="filterForm">
                    <div class="row g-4">
                        <div class="col-12 col-md-6 col-lg-3">
                            <label class="form-label fw-semibold text-uppercase small text-muted" for="start_date">Start Date</label>
                            <input type="date" id="start_date" name="start_date" class="form-control">
                        </div>
                        <div class="col-12 col-md-6 col-lg-3">
                            <label class="form-label fw-semibold text-uppercase small text-muted" for="end_date">End Date</label>
                            <input type="date" id="end_date" name="end_date" class="form-control">
                        </div>
                        <div class="col-12 col-lg-6">
                            <label class="form-label fw-semibold text-uppercase small text-muted" for="material_id">Material Grade</label>
                    ...
                    <div class="col-12 col-md-6 col-lg-3">
                        <label class="form-label fw-semibold text-uppercase small text-muted" for="currency_month">Currency Month</label>
                        <select id="currency_month" name="currency_month" class="form-select">
                            <option value="">YEN (Default)</option>
                        </select>
                    </div>
                    <div class="col-12 col-md-6 col-lg-3">
                        <label class="form-label fw-semibold text-uppercase small text-muted" for="currency_id">Phase</label>
                        <select id="currency_id" name="currency_id" disabled class="form-select">
                            <option value="">Pilih Bulan Kurs dulu</option>
                        </select>
                    </div>
                    <div class="col-12 col-md-6 col-lg-3">
                        <label class="form-label fw-semibold text-uppercase small text-muted" for="multiplier">Data Multiplier</label>
                    ...
                    <div class="mt-4 pt-4 border-top d-flex flex-wrap gap-3">
                        <button type="button" id="btnAddMaterial" data-bs-toggle="modal" data-bs-target="#materialModal" class="btn btn-warning">
                            <i class="bi bi-plus-circle me-2"></i>Add Material
                        </button>
                        <button type="button" id="btnAddCurrency" data-bs-toggle="modal" data-bs-target="#currencyModal" class="btn btn-danger">
                            <i class="bi bi-plus-circle me-2"></i>Add Currency
                        </button>
                        <button type="button" id="btnImportExport" data-bs-toggle="modal" data-bs-target="#importExportModal" class="btn btn-primary ms-auto">
                            <i class="bi bi-file-earmark-arrow-up me-2"></i>Import/Export
                        </button>
                        <span id="filterStatus" class="d-none align-items-center px-4 py-2 rounded badge bg-warning">
                ...
                <form id="currencyForm">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" id="kurs" name="kurs" 
                            placeholder="Contoh: 100" required step="0.01" min="0">
                        <small class="text-muted">1 YEN = berapa <span id="selectedCurrency">IDR</span>?</small>
                    </div>
...
jQuery(document).ready(function($) {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    // Tutup modal import/export
                    const importModalEl = document.getElementById('importExportModal');
                    const importModalInstance = bootstrap.Modal.getInstance(importModalEl);
    ...
    function showLoadingIndicator() {
        $('#filterStatus').removeClass('d-none bg-success')
            .addClass('d-flex bg-warning')
            .find('#filterStatusText').text('Sedang memuat data dari server...');
        $('#filterStatus .spinner-border').show();
    }
    
    // Hide loading indicator with success message
    function hideLoadingIndicator() {
        $('#filterStatus').removeClass('bg-warning')
            .addClass('bg-success')
    ...
    function renderTable(data) {
        ...
        data.forEach(function(material) {
            ...
            tbodyHtml += '<td class="table-secondary">FOB (Total)</td>';
            material.quarters.forEach(function(quarter, idx) {
                const year = extractYear(quarter.period);
                const bgClass = yearColorMap[year];
                tbodyHtml += '<td class="text-end fw-bold ' + bgClass + '">' + formatNumber(quarter.fob) + '</td>';
            });
            tbodyHtml += '</tr>';
            
            // Row 4: CNF
            tbodyHtml += '<tr>';
            tbodyHtml += '<td class="text-muted">CNF</td>';
            material.quarters.forEach(function(quarter, idx) {
                const year = extractYear(quarter.period);
                const bgClass = yearColorMap[year] ? yearColorMap[year] + ' bg-opacity-25' : '';
                tbodyHtml += '<td class="text-end text-muted ' + bgClass + '">' + formatNumber(quarter.cnf) + '</td>';
            });
            tbodyHtml += '</tr>';
