    ...
    <main id="main" class="main">
        @php
            ...
            $statsByType = $statsByType ?? [
                'new_product' => ['total' => 0, 'submitted' => 0, 'approved_1' => 0, 'approved_2' => 0, 'finished' => 0, 'cancelled' => 0],
                'update_price' => ['total' => 0, 'submitted' => 0, 'approved_1' => 0, 'approved_2' => 0, 'finished' => 0, 'cancelled' => 0],
            ];
            $statusClassMap = [
                'draft' => 'secondary',
            ...
            <div class="card itemcode-section-card">
                ...
                <div class="card-body pt-3">
                    ...
                    <div class="{{ $activeTab === 'new_product' ? '' : 'd-none' }}">
                        @php
                            $newStats = $statsByType['new_product'] ?? ['total' => 0, 'submitted' => 0, 'approved_1' => 0, 'approved_2' => 0, 'finished' => 0, 'cancelled' => 0];
                        ...
                        <div class="itemcode-toolbar">
                            ...
                            <div class="d-flex flex-wrap gap-2 align-items-center itemcode-toolbar-controls">
                                ...
                                <a href="{{ route('item-code.exportApproval', $newFilterParams) }}"
                                    ...
                                    data-filter-start-date="{{ $filters['start_date'] ?? '' }}"
                                    data-filter-end-date="{{ $filters['end_date'] ?? '' }}">
                                    Export
                                </a>
                    ...
                    <div class="{{ $activeTab === 'update_price' ? '' : 'd-none' }}">
                        @php
                            $updateStats = $statsByType['update_price'] ?? ['total' => 0, 'submitted' => 0, 'approved_1' => 0, 'approved_2' => 0, 'finished' => 0, 'cancelled' => 0];
...
.itemcode-table .action-cell {
    ...
    padding: 0.25rem 0.2rem !important;
        ...
        <script>
            ...
            initExportApprovalButtons();
            attachModalCleanupHandlers();

            function initExportApprovalButtons() {
                document.querySelectorAll('[data-export-approval]').forEach((button) => {
                    button.addEventListener('click', (event) => {
                        event.preventDefault();

                        const scope = button.getAttribute('data-bulk-scope') || '';
                        const filterQ = String(button.getAttribute('data-filter-q') || '').trim();
                        const filterStatus = String(button.getAttribute('data-filter-status') || '').trim();
                        const filterStartDate = String(button.getAttribute('data-filter-start-date') || '').trim();
                        const filterEndDate = String(button.getAttribute('data-filter-end-date') || '').trim();
                        const hasFilters = filterQ !== '' || filterStatus !== '' || filterStartDate !== '' || filterEndDate !== '';
