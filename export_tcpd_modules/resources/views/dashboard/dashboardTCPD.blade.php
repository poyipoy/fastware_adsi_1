...
@section('content')
                ...
                <div class="row g-3 mb-4" id="smart-insights-row" style="display: none;">
                    <div class="col-md-6">
                        <div class="card border-0 shadow-sm rounded-4 h-100" style="background: linear-gradient(135deg, #f0fdf4 0%, #dcfce7 100%);">
                            <div class="card-body p-3">
                                <h6 class="text-success fw-bold text-uppercase mb-2"><i class="bi bi-award-fill me-2"></i>Top 3 Job Positions</h6>
                    ...
                    <div class="col-md-6">
                        <div class="card border-0 shadow-sm rounded-4 h-100" style="background: linear-gradient(135deg, #fff1f2 0%, #ffe4e6 100%);">
                            <div class="card-body p-3">
                                <h6 class="text-danger fw-bold text-uppercase mb-2"><i class="bi bi-exclamation-octagon-fill me-2"></i>Critical Focus Area</h6>
                                <p class="small text-danger text-opacity-75 mb-3" style="font-size: 0.8rem;">Kompetensi dengan jumlah defisit terbanyak. <i class="bi bi-hand-index-thumb"></i> Klik untuk melihat detail karyawan.</p>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const competencyPagination = {
                ...
                render() {
                    ...
                    tbody.innerHTML = pageData.map((row, i) => {
                        ...
                        if (validMentors.length > 0) {
                            const badgesHtml = displayMentors.map(m => {
                                ...
                                return `<span class="badge bg-info bg-opacity-15 text-dark border border-info me-1 mb-1 d-inline-flex align-items-center" title="${titleInfo}" style="font-size: 0.72rem; font-weight: 500;">
                                    <i class="bi bi-person-check-fill text-info me-1"></i>${escapeHtml(m.name)}
                                </span>`;
                            }).join('');
                            const extraBadge = remainingCount > 0 ? `<span class="badge bg-secondary bg-opacity-15 text-secondary border border-secondary mb-1" style="font-size: 0.7rem;">+${remainingCount} mentor lainnya</span>` : '';
                    ...
                    if (pagesEl) {
                        ...
                        // Next
                        pagesHtml += `<li class="page-item ${this.currentPage === totalPages ? 'disabled' : ''}">
                    ...
                    if (paginationEl) {
                        if (showAll || totalPages <= 1) {
                            ...
                            // Tetap tampilkan info + per-page selector selama ada data
            ...
            // Inisialisasi pagination saat DOM siap
            competencyPagination.init();

            function updateCompetencyTable(competencies, tableBodyId = 'tcpd-competency-body') {
                // Flatten competencies to get all individual employee deficiencies
                const deficiencies = [];
                ...
                // Sort by employee name first, then by competency name
            ...
            function initDepartmentCharts(departments, startIndex = 0) {
                ...
                departments.forEach((dept, i) => {
                    ...
                    const option = {
                        ...
                        legend: { show: false },
                        grid: { top: '12%', left: '4%', right: '4%', bottom: hasYearMode ? '16%' : '14%', containLabel: true },
                        xAxis: {
                            type: 'category',
                            data: categories,
                            axisLabel: { interval: 0, rotate: hasYearMode || categories.length > 5 ? 20 : 0, hideOverlap: true },
                            axisTick: { alignWithLabel: true },
                        },
            ...
            function initJobPositionChart(entries, context = {}) {
                ...
                const option = {
                    ...
                    legend: { show: false },
                    grid: { top: '18%', left: '4%', right: '4%', bottom: '16%', containLabel: true },
                    xAxis: { type: 'category', data: categories, axisLabel: { interval: 0, rotate: categories.length > 5 ? 25 : 0, hideOverlap: true } },
                        ...
                        if (result.success && result.data) {
                            ...
                            if (meta.insights) {
                                ...
                                if (meta.insights.critical_focus && meta.insights.critical_focus.length > 0) {
                                    criticalFocusEl.innerHTML = meta.insights.critical_focus.map((comp) => {
                                ...
                                document.getElementById('smart-insights-row').style.display = 'none';
                            }
                            ...
                            if (meta.training_effectiveness && Object.keys(meta.training_effectiveness).length > 0 && meta.company_years && meta.company_years.length > 0) {
                                ...
                                document.getElementById('training-effectiveness-row').style.display = 'none';
                            }
