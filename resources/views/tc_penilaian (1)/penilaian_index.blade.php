            ...
            <div class="container">
                <div class="d-flex flex-wrap align-items-end justify-content-between gap-2 mb-3">
                    ...
                    <div class="d-flex flex-wrap align-items-end gap-2">
                        <form method="GET" action="{{ route('penilaian.index') }}" class="d-flex flex-column">
                            <label for="yearFilter" class="form-label mb-1">Periode Tahun</label>
                            <select id="yearFilter" name="year" class="form-select" onchange="this.form.submit()">
                                <option value="">Semua (3 Tahun Terakhir)</option>
                                @foreach ($availableYears as $yearOption)
                                    <option value="{{ $yearOption }}"
                                        {{ $selectedYear !== null && (int) $selectedYear === (int) $yearOption ? 'selected' : '' }}>
                                        {{ $yearOption }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <tbody>
                        ...
                        @foreach ($penilaianData as $item)
                            ...
                            <tr>
                                ...
                                <td>
                                    @if ($item->status == 1 && !in_array($userName, ['MARTINUS CAHYO RAHASTO','ARY RODJO PRASETYO','ANDIK TOTOK SISWOYO','YULMAI RIDO WINANDA','JESSICA PAUNE','SITI MARIA ULFA']))
