...
class PenilaianTCController extends Controller
{
    public function __construct(
        ...
        private CompetencyAssessmentService $competencyService
    ) {
    }
    ...
    public function getDetailCompetency(Request $request)
    {
        ...
        if ($id_user) {
            $workingExperienceData = WorkingExperience::where('user_id', $id_user)
                ->chronological()
                ->get()
                ->map(fn($we) => [
                    'id'           => $we->id,
        ...
        // --- Modul 2.3: Mentor Badges per Area Development Competency ---
        // Build badge data for each area development item
        $mentorBadges = [];
        if ($id_user) {
            $areaDev = $this->competencyService->getAreaDevelopmentCompetencies((int) $id_user);
            foreach ($areaDev as $comp) {
                $key = $comp['type'] . '_' . $comp['id'];
                $mentorBadges[$key] = $this->competencyService->getEmployeesMeetingStandard(
                    $comp['type'],
                    $comp['id'],
                    (int) $id_user
                );
