...
@section('warehouse-content')
    <div class="warehouse-dashboard-page" aria-labelledby="warehouse-dashboard-title">
        <x-warehouse.page-header title="Warehouse Dashboard" subtitle="Pantau kondisi stok dan aktivitas warehouse.">
            @if ($canStockIn)
                <a class="btn btn-primary" href="{{ route('warehouse.transactions.create', ['type' => 'IN']) }}">Stock In Baru</a>
            @endif
            @if ($canStockOut)
                <a class="btn btn-outline-primary" href="{{ route('warehouse.transactions.create', ['type' => 'OUT']) }}">Stock Out Baru</a>
            @endif
            @if ($canStockIn || $canStockOut)
                <a class="btn btn-outline-warning" href="{{ route('warehouse.transactions-used.create') }}">Stock In/Out Bekas</a>
            @endif
            @if ($canValidateStock)
                <a class="btn btn-outline-success" href="{{ route('warehouse.validations.index') }}">Validasi Stok</a>
            @endif
            @if ($canManageMaster)
            ...
            @endif
            @if ($canViewReport)
                <a class="btn btn-outline-secondary" href="{{ route('warehouse.reports.index') }}">Reporting</a>
            @endif
            @if ($canAdjust)
                <a class="btn btn-outline-danger" href="{{ route('warehouse.adjustments.create') }}">Penyesuaian</a>
            @endif
            @if ($canExport)
                <a class="btn btn-outline-secondary" href="{{ route('warehouse.exports.transactions') }}">Export</a>
            @endif
        ...
        <section class="warehouse-data-grid warehouse-data-grid-single" aria-label="Prioritas Warehouse">
            <x-warehouse.panel title="Perhatian Stok" subtitle="Prioritaskan pengisian ulang untuk item dengan stok menipis atau habis. Catatan tersimpan tetap tersedia saat stok kembali sehat.">
                <x-slot:header>
                    @if ($canStockIn)
                        <a href="{{ route('warehouse.transactions.create', ['type' => 'IN']) }}" class="btn btn-sm btn-outline-primary">Stock In</a>
                    @endif
                </x-slot:header>
                        ...
                        <tbody>
                            @forelse ($lowStock as $item)
                                <tr>
                                    ...
                                    <td>{{ \App\Services\Warehouse\WarehouseQuantity::display($item->current_stock) }}</td>
                                    ...
                                    <td>
                                        @if ($canUpdateStockAttention)
                                            <form method="POST" action="{{ route('warehouse.dashboard.stock-attention.update', $item) }}" class="d-flex align-items-start gap-2">
                                                @csrf
                                                @method('PATCH')
                                                <label class="visually-hidden" for="stock-attention-note-{{ $item->id }}">Catatan perhatian {{ $item->item_name }}</label>
                                                <input class="form-control form-control-sm" id="stock-attention-note-{{ $item->id }}" name="stock_attention_note" value="{{ $item->stock_attention_note }}" maxlength="255" placeholder="Tulis catatan">
                                                <button class="btn btn-sm btn-outline-secondary text-nowrap" type="submit">Simpan</button>
                                            </form>
                                        @else
                                            {{ $item->stock_attention_note ?: '—' }}
                                        @endif
                                    </td>
                                    @if ($canStockIn)
                                        <td><a class="btn btn-sm btn-outline-primary" href="{{ route('warehouse.transactions.create', ['type' => 'IN', 'barcode' => $item->barcode]) }}">Stock In</a></td>
                                    @endif
                                </tr>
                            @empty
                ...
                @if ($lowStock->hasPages())
                    <div class="warehouse-panel-pagination">{{ $lowStock->links('pagination::warehouse-bootstrap-5') }}</div>
                @endif
            </x-warehouse.panel>
        </section>
        ...
        <x-warehouse.panel class="warehouse-trend-panel" title="Analitik Pergerakan" subtitle="Satu periode untuk tren Stock In dan Stock Out.">
            ...
            <div class="warehouse-analytics-grid warehouse-analytics-grid-trend">
                <section class="warehouse-analytics-block" aria-labelledby="warehouse-trend-table-title">
                    ...
                    <div class="warehouse-table-wrap">
                        <table class="table warehouse-table" aria-label="Tren Stock In dan Stock Out">
                            ...
                            <tbody>@forelse ($trend as $date => $rows)<tr><td>{{ $date }}</td><td>{{ \App\Services\Warehouse\WarehouseQuantity::display($rows->get('IN')?->quantity) }}</td><td>{{ \App\Services\Warehouse\WarehouseQuantity::display($rows->get('OUT')?->quantity) }}</td></tr>@empty<tr><td colspan="3"><x-warehouse.empty-state title="Belum ada pergerakan" message="Tidak ada data pada periode ini." /></td></tr>@endforelse</tbody>
                        </table>
                    </div>
                </section>
            </div>
        ...
        <x-warehouse.panel class="warehouse-top-stock-out-panel" title="Agregasi Stock Out" subtitle="Peringkat item dan tipe mesin dipisahkan agar mudah dibandingkan.">
            <div class="warehouse-analytics-grid warehouse-analytics-grid-top-stock-out">
                <section class="warehouse-analytics-block warehouse-analytics-block-top-stock-out" aria-labelledby="warehouse-top-item-title">
                    ...
                    <div class="warehouse-chart-frame" style="--warehouse-chart-rows: {{ max(1, $topUsage->count()) }};"><canvas data-warehouse-bar-chart data-source="warehouse-top-item-data" role="img" aria-label="Grafik item dengan Stock Out terbanyak"></canvas></div>
                    <script type="application/json" id="warehouse-top-item-data">{!! json_encode(['labels' => $topUsage->map(fn ($usage) => (string) $usage->item_name)->values(), 'values' => $topUsage->map(fn ($usage) => (float) $usage->quantity)->values()], JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) !!}</script>
                    <details class="warehouse-chart-data"><summary>Lihat data tabel</summary><div class="warehouse-table-wrap"><table class="table warehouse-table" aria-label="Data Top Item Stock Out"><thead><tr><th scope="col">Nama Item</th><th scope="col">Jumlah</th></tr></thead><tbody>@forelse($topUsage as $usage)<tr><td>{{ $usage->item_name }}</td><td>{{ \App\Services\Warehouse\WarehouseQuantity::display($usage->quantity) }} {{ $usage->unit }}</td></tr>@empty<tr><td colspan="2">Belum ada data Stock Out item pada periode ini.</td></tr>@endforelse</tbody></table></div></details>
                </section>
                <section class="warehouse-analytics-block warehouse-analytics-block-top-stock-out" aria-labelledby="warehouse-top-machine-title">
                    <h3 id="warehouse-top-machine-title">Top Tipe Mesin Stock Out</h3>
                    <div class="warehouse-chart-frame" style="--warehouse-chart-rows: {{ max(1, $topMachineUsage->count()) }};"><canvas data-warehouse-bar-chart data-source="warehouse-top-machine-data" role="img" aria-label="Grafik tipe mesin dengan Stock Out terbanyak"></canvas></div>
                    <script type="application/json" id="warehouse-top-machine-data">{!! json_encode(['labels' => $topMachineUsage->map(fn ($usage) => (string) $usage->machine_type)->values(), 'values' => $topMachineUsage->map(fn ($usage) => (float) $usage->quantity)->values()], JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) !!}</script>
                    <details class="warehouse-chart-data"><summary>Lihat data tabel</summary><div class="warehouse-table-wrap"><table class="table warehouse-table" aria-label="Data Top Tipe Mesin Stock Out"><thead><tr><th scope="col">Tipe Mesin</th><th scope="col">Jumlah</th></tr></thead><tbody>@forelse($topMachineUsage as $usage)<tr><td>{{ $usage->machine_type }}</td><td>{{ \App\Services\Warehouse\WarehouseQuantity::display($usage->quantity) }}</td></tr>@empty<tr><td colspan="2">Belum ada data Stock Out tipe mesin pada periode ini.</td></tr>@endforelse</tbody></table></div></details>
                </section>
            </div>
        </x-warehouse.panel>
