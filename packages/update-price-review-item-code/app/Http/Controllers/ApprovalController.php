...
class ApprovalController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $itemsQuery = (clone $baseQuery)
            ...
            ->with(['creator', 'approver', 'approver2', 'finisher', 'canceller'])
    ...
    public function export(Request $request)
    {
        ...
        $filters = $this->resolveFilters($request);
        $selectedIds = array_values(array_unique(array_map('intval', $validated['selected_ids'] ?? [])));
        $hasFilters = !empty($filters['q']) || !empty($filters['status']) || !empty($filters['start_date']) || !empty($filters['end_date']);
        ...
        if ($hasFilters && !empty($selectedIds)) {
    ...
    private function buildBaseApprovalQuery(bool $isApprover1, bool $isApprover2, bool $isFinisher): Builder
    {
        ...
        // Finisher yang bukan approver hanya perlu lihat approved_2, finished, dan cancelled.
