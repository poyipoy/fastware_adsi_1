                    ...
                    <tbody>
                        ...
                        @foreach ($penilaianData as $item)
                            <tr>
                                ...
                                <td>
                                    @if ($item->status == 1)
                                        <span class="badge rounded-pill bg-info">Menunggu Persetujuan Section Head</span>
        ...
        <script>
            function kirimData(id_job_position) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '{{ route('update.status.depthead', ':id_job_position') }}'.replace(':id_job_position',
                                id_job_position),
                            type: 'POST',
                            data: {
                                _token: '{{ csrf_token() }}'
            ...
            function kirimData2(id_job_position) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '{{ route('update.status.approve', ':id_job_position') }}'.replace(
                                ':id_job_position',
                                id_job_position),
                            type: 'POST',
                            data: {
