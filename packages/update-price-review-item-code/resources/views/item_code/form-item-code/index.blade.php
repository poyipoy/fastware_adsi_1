...
@section('content')
    <main id="main" class="main">
        @php
            ...
            $isSpecialRequester = (bool) ($isSpecialRequester ?? false);
            $canImport = (bool) ($canImport ?? true);
            $canPriceReview = (bool) ($canPriceReview ?? false);
            ...
            $statsByType = $statsByType ?? [
                'new_product' => ['total' => 0, 'draft' => 0, 'pending_price_review' => 0, 'submitted' => 0, 'approved_1' => 0, 'approved_2' => 0, 'finished' => 0, 'cancelled' => 0],
                'update_price' => ['total' => 0, 'draft' => 0, 'pending_price_review' => 0, 'submitted' => 0, 'approved_1' => 0, 'approved_2' => 0, 'finished' => 0, 'cancelled' => 0],
            ];
            $statusClassMap = [
                'draft' => 'secondary',
                'pending_price_review' => 'warning text-dark border border-dark',
            ...
            <div class="card itemcode-section-card">
                ...
                <div class="card-body pt-3">
                    ...
                    <div class="{{ $activeTab === 'new_product' ? '' : 'd-none' }}">
                        @php
                            $newStats = $statsByType['new_product'] ?? ['total' => 0, 'draft' => 0, 'pending_price_review' => 0, 'submitted' => 0, 'approved_1' => 0, 'approved_2' => 0, 'finished' => 0, 'cancelled' => 0];
                        ...
                        <div class="itemcode-toolbar">
                            <div class="d-flex flex-wrap gap-2 itemcode-toolbar-stats">
                                ...
                                <span class="badge rounded-pill text-bg-warning border border-dark">Menunggu Input Harga: {{ $newStats['pending_price_review'] ?? 0 }}</span>
                                ...
                                <span class="badge rounded-pill text-bg-dark">Cancelled: {{ $newStats['cancelled'] }}</span>
                            </div>
                            ...
                            <div class="d-flex flex-wrap gap-2 align-items-center itemcode-toolbar-controls">
                                <form method="GET" action="{{ route('item-code.form') }}"
                                    ...
                                    <select class="form-select form-select-sm" name="status">
                                        ...
                                        <option value="pending_price_review" {{ $newSelectedStatus === 'pending_price_review' ? 'selected' : '' }}>Menunggu Input Harga</option>
                                        ...
                                        <option value="cancelled" {{ $newSelectedStatus === 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                                    </select>
                                ...
                                <div class="itemcode-toolbar-actions">
                                    ...
                                    @if ($canPriceReview)
                                        <a href="{{ route('item-code.price-review.index') }}" class="btn btn-sm btn-outline-warning">
                                            Review Harga Mamik
                                        </a>
                                    @endif
                                    ...
                                    <form action="{{ route('item-code.submitAll') }}" method="POST" class="d-inline"
                                        ...
                                        data-confirm-text="{{ $isSpecialRequester ? 'Data Draft milik Anda akan dikirim ke pemeriksaan harga Ilyas.' : 'Data Draft Produk Baru yang dipilih akan disubmit untuk approval.' }}">
                                        @csrf
                                        <input type="hidden" name="tab" value="new_product">
                                ...
                                <tbody>
                                    @forelse ($itemsNewProduct as $item)
                                        @php
                                            ...
                                            $isSpecialWorkflowItem = $item->type === 'new_product'
                                                && \App\Enums\ProcurementMenuAccessGroup::ITEM_CODE_SPECIAL_REQUESTER
                                                    ->hasAccess((string) optional($item->creator)->name);
                                            $canMutateItem = !$isSpecialRequester
                                                || ((int) $item->created_by === (int) auth()->id() && $item->type === 'new_product');
                                            ...
                                            } elseif ($item->status === 'pending_price_review') {
                                                $statusLabel = 'Menunggu Input Harga';
                                                $statusLabelHtml = e($statusLabel);
                                            } else {
                                        ...
                                        <tr>
                                            <td class="itemcode-select-cell">
                                                <input type="checkbox" class="form-check-input" data-bulk-select data-bulk-scope="new_product" value="{{ $item->id }}" aria-label="Pilih {{ $item->nomor_pengajuan ?: $item->product_code }}" {{ $item->status !== 'draft' || !$canMutateItem ? 'disabled' : '' }}>
                                            </td>
                                            <td>
                                            ...
                                            <td>{{ $item->unit ?: '-' }}</td>
                                            <td>{{ $item->currency }}</td>                                           
                                            <td class="text-end">{{ $item->price_per_pcs !== null ? number_format((float) $item->price_per_pcs, 2) : '-' }}</td>
                                            ...
                                            <td class="action-cell">
                                                @php
                                                    $detailItem = [
                                                        ...
                                                        'unit' => $item->unit,
                                                        'price_per_pcs' => $item->price_per_pcs !== null ? (float) $item->price_per_pcs : null,
                                                        ...
                                                        'canceller' => optional($item->canceller)->name,
                                                        'cancelled_at' => optional($item->cancelled_at)->format('d-m-Y H:i:s'),
                                                        'is_special_price_workflow' => $isSpecialWorkflowItem,
                                                    ];
                                                    $editItem = array_merge($item->toArray(), [
                                                        'is_special_price_workflow' => $isSpecialWorkflowItem,
                                                    ]);
                                                @endphp
                                                    ...
                                                    <div class="action-grid">
                                                        ...
                                                        @if ($item->status === 'draft' && $canMutateItem)
                                                            ...
                                                            <form action="{{ route('item-code.submit', $item->id) }}" method="POST"
                                                                ...
                                                                data-confirm-text="{{ $isSpecialWorkflowItem ? 'Data akan dikirim ke pemeriksaan harga Ilyas.' : 'Data akan disubmit untuk approval.' }}">
                                                                @csrf
                                                                <button type="submit" class="btn btn-sm btn-primary">Submit</button>
                                                            </form>
                                                        @elseif ($canCancel && in_array($item->status, ['approved_1', 'approved_2'], true))
                                                            <form action="{{ route('item-code.cancel', $item->id) }}" method="POST"
                                                                class="d-contents" data-cancel-item-form
                                                                data-confirm-text="Cancel bersifat final. Data tidak dapat diubah atau diproses kembali.">
                                                                @csrf
                    ...
                    <div class="{{ $activeTab === 'update_price' ? '' : 'd-none' }}">
                        @php
                            $updateStats = $statsByType['update_price'] ?? ['total' => 0, 'draft' => 0, 'pending_price_review' => 0, 'submitted' => 0, 'approved_1' => 0, 'approved_2' => 0, 'finished' => 0, 'cancelled' => 0];
                        ...
                        <div class="itemcode-toolbar">
                            <div class="d-flex flex-wrap gap-2 itemcode-toolbar-stats">
                                ...
                                <span class="badge rounded-pill text-bg-dark">Cancelled: {{ $updateStats['cancelled'] }}</span>
                            </div>
                            ...
                            <div class="d-flex flex-wrap gap-2 align-items-center itemcode-toolbar-controls">
                                <form method="GET" action="{{ route('item-code.form') }}"
                                    ...
                                    <select class="form-select form-select-sm" name="status">
                                        ...
                                        <option value="cancelled" {{ $updateSelectedStatus === 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                                    </select>
                            ...
                            <table id="update-price-table" class="table table-bordered table-striped align-middle itemcode-table">
                                ...
                                <tbody>
                                    @forelse ($itemsUpdatePrice as $item)
                                        ...
                                        <tr>
                                            <td class="itemcode-select-cell">
                                                <input type="checkbox" class="form-check-input" data-bulk-select data-bulk-scope="update_price" value="{{ $item->id }}" aria-label="Pilih {{ $item->nomor_pengajuan ?: $item->product_code }}" {{ $item->status !== 'draft' || !$canMutateItem ? 'disabled' : '' }}>
                                            </td>
                                            <td>
...
.action-grid .btn {
    ...
    text-align: center;
        ...
        <script>
            ...
            const itemcodeIsSpecialRequester = @json($isSpecialRequester);
            ...
            function initCancelItemAlerts() {
                const cancelForms = document.querySelectorAll('[data-cancel-item-form]');
                ...
                cancelForms.forEach((form) => {
                    form.addEventListener('submit', (event) => {
                        ...
                        const confirmText = form.getAttribute('data-confirm-text')
                            || 'Cancel bersifat final. Data tidak dapat diubah atau diproses kembali.';
            ...
            function setNewProductPriceReviewMode(enabled, currentPrice = null) {
                const priceWrap = document.getElementById('itemcode_new_price_standard_wrap');
                const notice = document.getElementById('itemcode_new_price_review_notice');
                const priceInput = document.getElementById('itemcode_new_price_per_pcs');
                const priceDisplay = document.getElementById('itemcode_new_reviewed_price_display');

                if (priceWrap) {
                    priceWrap.classList.toggle('d-none', enabled);
                }
                ...
                if (priceDisplay) {
                    const numericPrice = currentPrice !== null && currentPrice !== ''
                        ? Number(currentPrice)
                        : Number.NaN;
                    priceDisplay.textContent = Number.isFinite(numericPrice)
            ...
            function openCreateNewProduct(shouldAutofillNomor = true) {
                ...
                setNewProductPriceReviewMode(itemcodeIsSpecialRequester);
            ...
            function openEditNewProduct(button) {
                ...
                fillInput('itemcode_new_price_per_pcs', item.price_per_pcs !== null
                    && item.price_per_pcs !== undefined
                    && Number.isFinite(Number(item.price_per_pcs))
                    ? Number(item.price_per_pcs)
                    : '');
                setNewProductPriceReviewMode(
                    itemcodeIsSpecialRequester || Boolean(item.is_special_price_workflow),
                    item.price_per_pcs
                );
            ...
            function setStatusBadge(statusRawValue, labelHtml) {
                ...
                const classMap = {
                    ...
                    pending_price_review: 'text-bg-warning text-dark border border-dark',
            ...
            function openDetailModal(button) {
                ...
                setDetail('detail_canceller', item.canceller || '-');
                setDetail('detail_cancelled_at', item.cancelled_at || '-');
            }
            ...
            function getStatusBadgeClass(status) {
                ...
                switch (normalized) {
                    case 'draft':
                        return 'bg-secondary';
                    case 'menunggu input harga':
                    case 'pending_price_review':
                        return 'bg-warning text-dark border border-dark';
                    case 'submitted':
