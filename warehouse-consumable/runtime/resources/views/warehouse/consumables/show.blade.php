...
@section('warehouse-content')
    <div class="warehouse-management-page" aria-labelledby="warehouse-consumable-detail-title">
        ...
        <div class="warehouse-detail-grid">
            <x-warehouse.panel title="Informasi barang" class="warehouse-detail-full"><dl class="warehouse-definition-list">
                @if($consumable->photo_path)<dt>Foto</dt><dd><img class="warehouse-master-photo-preview" src="{{ \Illuminate\Support\Facades\Storage::disk(config('warehouse.photos.disk', 'public'))->url($consumable->photo_path) }}" alt="Foto {{ $consumable->item_name }}" width="320" height="220"></dd>@endif
                <dt>Item Code</dt><dd class="font-monospace">{{ $consumable->item_code }}</dd>
                <dt>Nama barang</dt><dd>{{ $consumable->item_name }}</dd>
                <dt>Tipe mesin</dt><dd>{{ $consumable->machine_type ?: '—' }}</dd>
                ...
                <dt>DS8 — Baru</dt><dd>{{ \App\Services\Warehouse\WarehouseQuantity::display($consumable->availableAt('DS8', \App\Enums\Warehouse\WarehouseItemCondition::NEW)) }}</dd>
                <dt>DS8 — Bekas</dt><dd>{{ \App\Services\Warehouse\WarehouseQuantity::display($consumable->stock_used_ds8) }}</dd>
                <dt>DS8 — Total</dt><dd>{{ \App\Services\Warehouse\WarehouseQuantity::display($consumable->stock_ds8) }}</dd>
                <dt>Deltamas — Baru</dt><dd>{{ \App\Services\Warehouse\WarehouseQuantity::display($consumable->availableAt('Deltamas', \App\Enums\Warehouse\WarehouseItemCondition::NEW)) }}</dd>
                <dt>Deltamas — Bekas</dt><dd>{{ \App\Services\Warehouse\WarehouseQuantity::display($consumable->stock_used_deltamas) }}</dd>
                <dt>Deltamas — Total</dt><dd>{{ \App\Services\Warehouse\WarehouseQuantity::display($consumable->stock_deltamas) }}</dd>
