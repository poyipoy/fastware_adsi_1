...
@section('content')
                                ...
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <div id="export-button-container">
                                        @if ($selectedJobPositionId)
                                            <a href="{{ route('dashboardTCPD.export', ['job_position_id' => $selectedJobPositionId, 'department' => $selectedDepartment, 'date_from' => $jobDateFrom, 'date_to' => $jobDateTo]) }}" class="btn btn-sm btn-outline-primary">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const endpoints = {!! json_encode(['company' => route('dashboardTCPD.companyData'), 'job' => route('dashboardTCPD.data'), 'detail' => route('dsDetailCompetency'), 'export' => route('dashboardTCPD.export')]) !!};
            ...
            const updateExportLink = () => {
                ...
                if (!container || !endpoints.export) {
                    if (container) container.innerHTML = '';
                    return;
                }
                const jobPositionId = jobSelect ? jobSelect.value : null;
                if (!jobPositionId) {
                    container.innerHTML = '';
                    return;
                }
                const department = jobDepartmentSelect ? jobDepartmentSelect.value : '';
                const dateFrom = jobDateFromInput ? jobDateFromInput.value : '';
                const dateTo = jobDateToInput ? jobDateToInput.value : '';
                try {
                    const url = new URL(endpoints.export, window.location.origin);
                    url.searchParams.set('job_position_id', jobPositionId);
                    url.searchParams.set('department', department);
                    url.searchParams.set('date_from', dateFrom);
                    url.searchParams.set('date_to', dateTo);
                    container.innerHTML = `
                        <a href="${url.toString()}" class="btn btn-sm btn-outline-primary">
                            <i class="bi bi-download"></i> Export Excel
                        </a>
                    `;
                } catch (e) {
                    container.innerHTML = '';
                    console.error('Failed to construct export URL', e);
                }
            };
