...
class ItemCodeController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $itemsQuery = ItemCode::with(['creator', 'priceReviewer', 'approver', 'approver2', 'finisher', 'canceller'])
            ->withCount([
        ...
        return view('item_code.form-item-code.index', [
            ...
            'isSpecialRequester' => $this->isCurrentUserSpecialRequester(),
            'canImport' => !$this->isCurrentUserSpecialRequester(),
            'canPriceReview' => ProcurementMenuAccessGroup::ITEM_CODE_PRICE_REVIEWER->hasAccess($this->currentUserName()),
        ]);
    }
    ...
    {
        $this->ensureFormUser();
        $this->ensureSpecialRequesterCanCreate((string) $request->input('type'));
    ...
    public function update(Request $request, int $id): RedirectResponse
    {
        ...
        $this->ensureSpecialRequesterCanMutate($itemCode);
    ...
    public function submit(int $id): RedirectResponse
    {
        ...
        $itemCode = ItemCode::with('creator')->findOrFail($id);
        $this->ensureSpecialRequesterCanMutate($itemCode);

        $targetStatus = $this->resolveSubmissionTargetStatus($itemCode);

        abort_if(
            !$itemCode->canTransitionTo($targetStatus),
            403,
            'Transisi status tidak valid: hanya Draft yang bisa disubmit.'
        );

        $statusFrom = $itemCode->status;
        DB::transaction(function () use ($itemCode, $statusFrom, $targetStatus): void {
            $itemCode->update($this->buildSubmissionPayload($itemCode, $targetStatus));

            $itemCode->refresh();

            $isPriceReview = $targetStatus === ItemCode::STATUS_PENDING_PRICE_REVIEW;
            $this->historyService()->record(
                $itemCode,
                $isPriceReview
                    ? ItemCodeHistoryService::ACTION_PRICE_REVIEW_REQUESTED
                    : ItemCodeHistoryService::ACTION_SUBMITTED,
                $isPriceReview
                    ? 'Pengajuan dikirim kepada ILYAS NOOR FIRDAUS untuk pemeriksaan dan pengisian harga.'
        ...
        return redirect()
            ...
            ->with(
                'success',
                $targetStatus === ItemCode::STATUS_PENDING_PRICE_REVIEW
                    ? 'Data berhasil dikirim ke antrean pemeriksaan harga Ilyas.'
                    : 'Data item code berhasil disubmit.'
            );
    }
    ...
    public function submitAll(Request $request): RedirectResponse
    {
        ...
        if ($this->isCurrentUserSpecialRequester() && $tab !== ItemCode::TYPE_NEW_PRODUCT) {
            abort(403, 'MAMIK ABIDIN hanya dapat mensubmit pengajuan Produk Baru.');
        }
        ...
        $draftItems = ItemCode::query()
            ...
            ->when(
                $this->isCurrentUserSpecialRequester(),
        ...
        DB::transaction(function () use ($draftItems) {
            foreach ($draftItems as $itemCode) {
                ...
                if (!$itemCode->canTransitionTo($targetStatus)) {
                    continue;
                }
        ...
        return redirect()
            ->route('item-code.form', ['tab' => $tab])
            ->with('success', $draftItems->count() . ' data Draft berhasil diproses.');
    }
    ...
    public function priceReview(Request $request): View
    {
        $this->ensurePriceReviewer();

        $perPage = $this->resolvePerPage($request->query('per_page'));
        $baseQuery = $this->buildPriceReviewQuery();
        $pendingCount = (clone $baseQuery)->count();

        $items = $baseQuery
            ->with(['creator', 'priceReviewer'])
            ->orderBy('tanggal')
            ->orderBy('id')
            ->simplePaginate($perPage)
            ->appends($request->query());

        return view('item_code.price-review.index', [
            'items' => $items,
    ...
    public function confirmPrice(Request $request, int $id): RedirectResponse
    {
        $this->ensurePriceReviewer();
        ...
        DB::transaction(function () use ($id, $validated): void {
            ...
            $this->ensurePriceReviewItem($itemCode);

            abort_if(
                !$itemCode->canTransitionTo(ItemCode::STATUS_SUBMITTED),
                403,
                'Transisi status review harga tidak valid.'
            );

            $statusFrom = (string) $itemCode->status;
            $oldPrice = $itemCode->price_per_pcs;
            $oldReviewer = optional($itemCode->priceReviewer)->name;
            $oldReviewedAt = optional($itemCode->price_reviewed_at)->format('Y-m-d H:i:s');
            $reviewedAt = now();

            $itemCode->update([
                'price_per_pcs' => (string) $validated['price_per_pcs'],
                'price_reviewed_by' => Auth::id(),
                'price_reviewed_at' => $reviewedAt,
            ...
            $this->historyService()->record(
                $itemCode,
                ItemCodeHistoryService::ACTION_PRICE_REVIEW_COMPLETED,
                'Harga diisi dan dikonfirmasi oleh ILYAS NOOR FIRDAUS.',
                $statusFrom,
                $itemCode->status,
                [
                    [
                        ...
                        'label' => 'Price',
                        'old' => $oldPrice,
                        'new' => $itemCode->price_per_pcs,
                    ],
                    [
                        'field' => 'price_reviewed_by',
                        'label' => 'Pemeriksa Harga',
                        'old' => $oldReviewer,
                        'new' => $this->currentUserName(),
                    ],
                    [
                        'field' => 'price_reviewed_at',
                        'label' => 'Waktu Pemeriksaan Harga',
                        'old' => $oldReviewedAt,
                        'new' => $reviewedAt->format('Y-m-d H:i:s'),
                    ],
                    ...$this->buildStatusChangeSet($statusFrom, $itemCode->status),
                ]
            );
        ...
        return redirect()
            ->route('item-code.price-review.index')
            ->with('success', 'Harga berhasil dikonfirmasi dan pengajuan dikirim ke proses approval.');
    }

    public function returnPriceReview(Request $request, int $id): RedirectResponse
    {
        $this->ensurePriceReviewer();
        ...
        DB::transaction(function () use ($id, $validated): void {
            ...
            abort_if(
                ...
                403,
                'Transisi pengembalian review harga tidak valid.'
            );

            $statusFrom = (string) $itemCode->status;
            $returnReason = trim((string) $validated['return_reason']);
            ...
            $this->historyService()->record(
                $itemCode,
                ItemCodeHistoryService::ACTION_PRICE_REVIEW_RETURNED,
                'Pengajuan dikembalikan kepada MAMIK ABIDIN untuk diperbaiki.',
                $statusFrom,
                $itemCode->status,
                [
                    ...$this->buildStatusChangeSet($statusFrom, $itemCode->status),
        ...
        return redirect()
            ->route('item-code.price-review.index')
            ->with('success', 'Pengajuan berhasil dikembalikan ke Mamik sebagai Draft.');
    }
    ...
    {
        $this->ensureFormUser();
        $this->ensureStandardFormUser('Import Item Code tidak tersedia untuk MAMIK ABIDIN.');
    ...
    {
        $this->ensureFormUser();
        $this->ensureStandardFormUser('Template import Item Code tidak tersedia untuk MAMIK ABIDIN.');
    ...
    public function nextNomor(Request $request): JsonResponse
    {
        ...
        if ($this->isCurrentUserSpecialRequester() && $validated['type'] !== ItemCode::TYPE_NEW_PRODUCT) {
            abort(403, 'MAMIK ABIDIN hanya dapat membuat pengajuan Produk Baru.');
        }
    ...
    {
        $priceProtected = $this->shouldProtectPrice($request, $existingItem);

        if ($existingItem !== null
            && $this->isSpecialNewProduct($existingItem)
            && (string) $request->input('type') !== ItemCode::TYPE_NEW_PRODUCT) {
            throw ValidationException::withMessages([
                'type' => 'Jenis pengajuan khusus Mamik tidak dapat diubah dari Produk Baru.',
            ]);
        }
        ...
        $data = $request->validate([
            ...
            'price_per_pcs' => $priceProtected ? 'nullable' : 'required|numeric|min:0',
        ...
        $data['price_per_pcs'] = $priceProtected
            ? $existingItem?->price_per_pcs
            : (float) $data['price_per_pcs'];
    ...
    {
        $hasStandardAccess = ProcurementMenuAccessGroup::ITEM_CODE_FORM->hasAccess($this->currentUserName());
        $hasSpecialAccess = ProcurementMenuAccessGroup::ITEM_CODE_SPECIAL_REQUESTER->hasAccess($this->currentUserName());

        if (!$hasStandardAccess && !$hasSpecialAccess) {
            abort(403, 'Unauthorized');
        }
    }

    private function ensureStandardFormUser(string $message = 'Unauthorized'): void
    {
    ...
    private function ensureHistoryAccess(ItemCode $itemCode, bool $json = false): void
    {
        ...
        $hasSpecialRequesterAccess = ProcurementMenuAccessGroup::ITEM_CODE_SPECIAL_REQUESTER
            ->hasAccess($this->currentUserName());

        if ($isOwner || $hasApprovalAccess || $hasFormAccess || $hasSpecialRequesterAccess) {
            return;
        }
    ...
    private function statusLabel(string $status): string
    {
        return match (strtolower($status)) {
            ...
            ItemCode::STATUS_PENDING_PRICE_REVIEW => 'Menunggu Input Harga',
    ...
    private function historyFieldLabels(): array
    {
        return [
            ...
            'status' => 'Status',
            'price_reviewed_by' => 'Pemeriksa Harga',
            'price_reviewed_at' => 'Waktu Pemeriksaan Harga',
        ];
    }
    ...
    private function buildSubmissionPayload(ItemCode $itemCode, string $targetStatus): array
    {
    ...
    private function resolveSubmissionTargetStatus(ItemCode $itemCode): string
    {
        return $this->isSpecialNewProduct($itemCode)
            ? ItemCode::STATUS_PENDING_PRICE_REVIEW
            : ItemCode::STATUS_SUBMITTED;
    }

    private function buildPriceReviewQuery(): Builder
    {
        $requesterNames = array_map(
            static fn (string $name): string => strtoupper(trim($name)),
            ProcurementMenuAccessGroup::ITEM_CODE_SPECIAL_REQUESTER->getAllowedUsers()
        );

        return ItemCode::query()
            ->where('type', ItemCode::TYPE_NEW_PRODUCT)
            ->where('status', ItemCode::STATUS_PENDING_PRICE_REVIEW)
            ->whereHas('creator', function (Builder $query) use ($requesterNames): void {
                $query->whereIn(DB::raw('UPPER(TRIM(name))'), $requesterNames);
            });
    }

    private function shouldProtectPrice(Request $request, ?ItemCode $existingItem): bool
    {
        if ($existingItem !== null) {
            return $this->isSpecialNewProduct($existingItem);
        }

        return $this->isCurrentUserSpecialRequester()
            && (string) $request->input('type') === ItemCode::TYPE_NEW_PRODUCT;
    }

    private function isSpecialNewProduct(ItemCode $itemCode): bool
    {
        if ($itemCode->type !== ItemCode::TYPE_NEW_PRODUCT) {
            return false;
        }

        $itemCode->loadMissing('creator');

        return ProcurementMenuAccessGroup::ITEM_CODE_SPECIAL_REQUESTER
            ->hasAccess((string) optional($itemCode->creator)->name);
    }

    private function isCurrentUserSpecialRequester(): bool
    {
        return ProcurementMenuAccessGroup::ITEM_CODE_SPECIAL_REQUESTER
            ->hasAccess($this->currentUserName());
    }

    private function ensureSpecialRequesterCanCreate(string $type): void
    {
        if ($this->isCurrentUserSpecialRequester() && $type !== ItemCode::TYPE_NEW_PRODUCT) {
            abort(403, 'MAMIK ABIDIN hanya dapat membuat pengajuan Produk Baru.');
        }
    }

    private function ensureSpecialRequesterCanMutate(ItemCode $itemCode): void
    {
        if (!$this->isCurrentUserSpecialRequester()) {
            return;
        }

        abort_unless(
            (int) $itemCode->created_by === (int) Auth::id()
                && $itemCode->type === ItemCode::TYPE_NEW_PRODUCT,
            403,
            'MAMIK ABIDIN hanya dapat memproses pengajuan Produk Baru miliknya sendiri.'
        );
    }

    private function ensurePriceReviewer(): void
    {
        abort_unless(
            ProcurementMenuAccessGroup::ITEM_CODE_PRICE_REVIEWER->hasAccess($this->currentUserName()),
            403,
            'Unauthorized: hanya ILYAS NOOR FIRDAUS yang dapat memproses review harga.'
        );
    }

    private function ensurePriceReviewItem(ItemCode $itemCode): void
    {
        abort_unless(
            $itemCode->status === ItemCode::STATUS_PENDING_PRICE_REVIEW
                && $this->isSpecialNewProduct($itemCode),
            403,
            'Item ini tidak memenuhi syarat antrean review harga Mamik.'
        );
    }
