            ...
            <div class="om-table-wrap"><table id="outstandingInvoiceDetailTable" class="om-table">
                <thead>
                    <tr class="om-filter-row">
                        <th></th>
                        <th><select id="filter_supplier" class="om-inline-filter"><option value="">All</option>@foreach ($filterOptions['suppliers'] as $value)<option value="{{ $value }}">{{ $value }}</option>@endforeach</select></th>
                        <th><select id="filter_type" class="om-inline-filter"><option value="">All</option>@foreach ($filterOptions['types'] as $value)<option value="{{ $value }}">{{ $value }}</option>@endforeach</select></th>
                        <th><input id="filter_thickness" class="om-inline-filter"></th><th><input id="filter_width" class="om-inline-filter"></th><th><input id="filter_diameter" class="om-inline-filter"></th><th><input id="filter_material_length" class="om-inline-filter"></th><th><input id="filter_qty_pcs" class="om-inline-filter"></th><th><input id="filter_est_qty_kg" class="om-inline-filter"></th>
                        <th><span class="text-muted small">Locked</span></th>
                        <th><select id="filter_status" class="om-inline-filter"><option value="">All</option>@foreach ($statusOptions as $value)<option value="{{ $value }}">{{ $value }}</option>@endforeach</select></th>
                        ...
                        <th><select id="filter_port" class="om-inline-filter"><option value="">All</option>@foreach (\App\Models\OutstandingMaterial::portOptions() as $value)<option value="{{ $value }}">{{ $value }}</option>@endforeach</select></th>
                        <th><input id="filter_number_po" class="om-inline-filter" placeholder="..."></th>
                        <th><input id="filter_remarks" class="om-inline-filter" placeholder="..."></th>
                        <th></th><th></th><th></th>
                    </tr>
                    <tr class="om-column-header">
                        <th>NO</th><th>Supplier</th><th>TYPE</th><th>Thickness</th><th>Width</th><th>Diameter</th><th>Length</th><th>QTY<br>(PCS)</th><th>Est QTY<br>(KG)</th><th>Number<br>Invoice</th><th>Status</th><th>ETA Port</th><th>ETA Warehouse</th><th>Bulan ETA</th><th>Keterangan</th><th>Delay Port</th><th>Delay Warehouse</th><th>Port</th><th>Nomor PO</th><th>Remarks</th><th>Packing List</th><th>MTC</th><th>Action</th>
                    </tr>
                </thead><tbody></tbody>
            </table></div>
        </div></div>
...
@if ($canManageOutstandingMaterials)
<div class="modal fade" id="importOutstandingMaterialModal" tabindex="-1" aria-hidden="true"><div class="modal-dialog"><div class="modal-content"><form method="POST" action="{{ route('outstanding-materials.import') }}" enctype="multipart/form-data">@csrf<div class="modal-header"><h5 class="modal-title">Import Multi-Invoice</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body"><div class="mb-3"><label class="form-label d-block fw-semibold">Mode Import</label><div class="form-check form-check-inline"><input class="form-check-input" type="radio" name="mode" id="import_mode_add_show" value="add" checked><label class="form-check-label" for="import_mode_add_show"><strong>Add</strong> (Tambah baru)</label></div><div class="form-check form-check-inline"><input class="form-check-input" type="radio" name="mode" id="import_mode_replace_show" value="replace"><label class="form-check-label" for="import_mode_replace_show"><strong>Replace</strong> (Update yang cocok)</label></div></div><div class="mb-2"><label for="import_file" class="form-label fw-semibold">File Excel / CSV</label><input type="file" class="form-control" id="import_file" name="import_file" accept=".xlsx,.xls,.csv" required><div class="form-text mt-1">Satu file dapat berisi banyak supplier, invoice, dan material. Preview akan muncul sebelum data disimpan.</div></div></div><div class="modal-footer"><button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button><button type="submit" class="btn btn-primary">Preview Import</button></div></form></div></div></div>
@endif
@endsection
