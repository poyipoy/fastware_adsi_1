...
class TcpdEmployeesExport implements FromArray, ShouldAutoSize, WithColumnWidths, WithTitle, WithEvents
{
    ...
    public function array(): array
    {
        ...
        foreach ($this->criticalFocus as $comp) {
            ...
            $formula = $empCount > 0 ? "=COUNTA(D{$startRow}:D{$endRow})" : 0;
        ...
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                if ($this->topJobsEndRow >= $this->topJobsHeaderRow) {
                    // Green Header
                    $sheet->getStyle("A{$this->topJobsHeaderRow}:H{$this->topJobsHeaderRow}")->applyFromArray([
                ...
                if ($this->criticalEndRow >= $this->criticalHeaderRow) {
                    // Red Header
                    $sheet->getStyle("A{$this->criticalHeaderRow}:H{$this->criticalHeaderRow}")->applyFromArray([
                        'font' => ['bold' => true, 'color' => ['argb' => 'FFFFFFFF']],
                        'fill' => [
                            'fillType'   => Fill::FILL_SOLID,
                    ...
                    if ($this->criticalEndRow > $this->criticalHeaderRow) {
                        ...
                        // Number Format (0.00)
                        $sheet->getStyle("E{$start}:F{$end}")->getNumberFormat()->setFormatCode('0.00');
