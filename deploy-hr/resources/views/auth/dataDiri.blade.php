...
<main id="main" class="main">
            ...
            <div class="col-xl-12">
                <div class="card">
                    <div class="card-body profile-card pt-4 d-flex flex-column align-items-center">
                        @if ($user->file)
                        <img src="{{ asset('assets/data_diri/' . $user->file) }}" alt="Profile" class="rounded-circle"
                            style="width: 150px; height: 120px; cursor: pointer;" data-bs-toggle="modal"
                        ...
                        <img src="{{ asset('assets/img/user.png') }}" alt="Default Profile" class="rounded-circle"
                            ...
                            data-bs-target="#imageModal">
                        @endif
                        <h2 class="fonav">{{ Auth::user()->name }} - {{ Auth::user()->km_total_poin }}
                            <span class="fs-5">Poin</span>
                            <br>
                        </h2>

                        <span class="fonav fs-6 fw-bold">--- {{ Auth::user()->roles->role }} ---</span>
                        <h3 class="fonav">
                    ...
                    <div class="card-body pt-3 fonav">
                        <!-- Bordered Tabs -->
                        ...
                        <div class="tab-content pt-2">
                            ...
                            <div class="tab-pane fade show active profile-overview" id="profile-overview">
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control" value="{{ $user->section }}" disabled>
                                    </div>
                                </div>
                                {{-- <div class="row mb-3">
                                    <label class="col-sm-2 col-form-label">Posisi</label>
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control" value="@foreach ($jobPositions as $position)
                                                {{ $position->job_position }} @endforeach" disabled>
                                    </div>
                                </div> --}}
                                <div class="row mb-3">
                                    <label class="col-sm-2 col-form-label">Posisi</label>
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control"
                                            value="{{ implode(', ', $jobPositions->pluck('job_position')->toArray()) }}"
                                            disabled>
                                    </div>
                                </div>
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control" value="{{ $user->npk }}" disabled>
                                    </div>
                                </div>
                                <div class="row mb-3">
                    ...
                    <div style="overflow-x: auto; white-space: nowrap;">
                        <table border="1" cellpadding="5" cellspacing="0" class="table table-bordered mt-3">
                            ...
                            <tbody id="peopleDevTabel">
                                ...
                                @else
                                @foreach ($dataTcPeopleDevelopment as $evaluation)
                                <tr>
                                    <td class="text-start">
                                        @if ($evaluation['is_sharing_knowledge'])
                                            <span class="badge bg-primary mb-1">Sharing Knowledge</span>
                                            @foreach ($evaluation['participants'] as $participant)
                                    ...
                                    <td>{{ $evaluation['program_training_plan'] }}</td>
                                    <td>{{ $evaluation['kategori_competency'] }}</td>
                                    <td>{{ $evaluation['due_date_plan'] }}</td>
                                    <td>
                                        <button type="button" onclick="downloadPdf({{ $evaluation['id'] }})"
                                            aria-label="Unduh bukti {{ $evaluation['program_training_plan'] }}"
                                            style="cursor: pointer; background-color: transparent; border: none; color: blue; text-decoration: underline;">
                                            <i class="bi bi-download"></i>
                                        </button>
                                        <button type="button" class="btn btn-primary btn-sm evaluation-view-button"
                                            data-evaluation-id="{{ $evaluation['id'] }}">
                                            <i class="fas fa-eye"></i> View Evaluasi
                                        </button>

                                        <button type="button" data-evaluation-id="{{ $evaluation['id'] }}"
                                            class="btn btn-success btn-sm evaluation-print-button">
                                            <i class="fas fa-file-pdf" aria-hidden="true"></i> Print PDF
                                        </button>
                                    </td>
                                </tr>
                                @endforeach
                        ...
                        <form>
                            ...
                            <div class="card my-4">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="mb-3">
                                        <label for="alasan_relevansi" class="form-label"><strong>Alasan</strong></label>
                                        <textarea id="alasan_relevansi" class="form-control" rows="3"
                                            readonly></textarea>
                                    </div>
                                    <div class="mb-3">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <textarea id="alasan_rekomendasi" class="form-control" rows="3"
                                            readonly></textarea>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="card-body">
                                    <div class="row mb-3">
                                        <div class="col-md-6">
                                            <label for="kelengkapan_materi" class="form-label"><strong>Kelengkapan
                                                    Materi</strong></label>
                                            <input type="text" id="kelengkapan_materi" class="form-control" readonly>
                                        </div>
                                        <div class="col-md-6">
                                    ...
                                    <div class="row mb-3">
                                        <div class="col-md-6">
                                            ...
                                            <input type="text" id="metode_pengajaran" class="form-control" readonly>
                                        </div>
                                        <div class="col-md-6">
                            ...
                            <div class="card my-4">
                                ...
                                <div class="card-body">
                                    <div class="mb-3">
                                        ...
                                        <textarea id="catatan_tambahan" class="form-control" rows="3"
                                            readonly></textarea>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="card my-4">
                                ...
                                <div class="card-body">
                                    <div class="row mb-3">
                                        <div class="col-md-6">
                                            <label for="dievaluasi" class="form-label">
                                                <strong id="evaluation-participant-signature-label">Diketahui oleh:</strong>
                                            </label>
                                            <br>
    ...
    <script>
        const trainingEvaluations = @json($dataTcPeopleDevelopment);
        const trainingEvaluationById = new Map(
            trainingEvaluations.map((evaluation) => [Number(evaluation.id), evaluation]),
        );
        ...
        function validatePasswordChange() {
            ...
            function showModal(data) {
                const participantLabels = (data.participants || [])
                    .map((participant) => participant.label || `${participant.npk || '-'} — ${participant.name || '-'}`);

                // Isi field modal dengan data dari baris yang dipilih
                document.getElementById('section').value = data.section || '';
                document.getElementById('npk').value = data.is_sharing_knowledge
                    ? (data.participants || []).map((participant) => participant.npk || '-').join(', ')
                    : (data.user?.npk || '-');
                document.getElementById('nama').value = data.is_sharing_knowledge
                ...
                document.getElementById('evaluation-participant-signature-label').innerText =
                    data.is_sharing_knowledge ? 'Participant kegiatan:' : 'Diketahui oleh:';
                document.getElementById('dievaluasi').innerText = data.is_sharing_knowledge
                    ? participantLabels.join(', ')
                    : (data.diketahui || data.user?.label || '-');
                document.getElementById('diketahui').innerText = data.dievaluasi || '-';
            ...
            document.querySelectorAll('.evaluation-view-button').forEach((button) => {
                button.addEventListener('click', function() {
                    const evaluation = trainingEvaluationById.get(Number(this.dataset.evaluationId));
                    if (evaluation) showModal(evaluation);
                });
            });

            document.querySelectorAll('.evaluation-print-button').forEach((button) => {
                button.addEventListener('click', async function() {
                    const id = Number(this.dataset.evaluationId);
                    ...
                    try {
                        ...
                        if (!response.ok) {
                            throw new Error(response.status === 403
                                ? 'Anda tidak memiliki akses ke evaluasi ini.'
                                : 'Data evaluasi belum dapat diambil.');
                        }

                        window.TrainingEvaluationPdf.download(await response.json());
                    } catch (error) {
                        Swal.fire({
                            icon: 'error',
