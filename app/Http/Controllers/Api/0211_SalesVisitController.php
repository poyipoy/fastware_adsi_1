...
class SalesVisitController extends Controller
{
    ...
    public function chartadmin(Request $request)
    {
        ...
        $usersMap = User::whereIn('name', $sales)->pluck('id', 'name');
        $salesUserIds = $usersMap->values()->all();
        ...
        if (empty($salesUserIds)) {
            ...
            $totalKunjungan = LogbookVisits::whereIn('id_user', $salesUserIds)
                ->whereBetween('visit_date', [$startDateCarbon, $endDateCarbon])
                ->count();

            $totalPlan = TrsLogbookVisits::whereIn('id_user', $salesUserIds)
                ->whereBetween('plan_visit', [$startDateCarbon, $endDateCarbon])
                ->count();

            $followUp = LogbookVisits::whereIn('id_user', $salesUserIds)
                ->where('remark', 'Follow Up')
                ->whereBetween('visit_date', [$startDateCarbon, $endDateCarbon])
                ->count();

            $oldCustomer = LogbookVisits::whereIn('id_user', $salesUserIds)
                ->whereNull('new_customer_name')
                ->whereNotNull('customer_name')
                ->whereBetween('visit_date', [$startDateCarbon, $endDateCarbon])
                ->count();

            $newCustomer = LogbookVisits::whereIn('id_user', $salesUserIds)
                ->whereNotNull('new_customer_name')
                ->whereBetween('visit_date', [$startDateCarbon, $endDateCarbon])
                ->count();
        ...
        if (!isset($visitsAgg)) $visitsAgg = collect();
        if (!isset($plansAgg)) $plansAgg = collect();
